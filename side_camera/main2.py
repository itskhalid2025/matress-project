...
Integrated Subsystems:
  1. Live MJPEG Video Stream (webcam / side camera).
  2. QR Code Decoding (pyzbar with multi-stage retry ladder).
  3. Side Label OCR Reading (pytesseract + label_reference.json lookup).
  4. Multi-Way Reconciliation (Policy check across QR and Label).
  5. On-Demand Computation: Process button triggers analysis on current frame.
  6. Results Panel: Renders all outputs and verdict on the right panel.
"""
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/", "/index.html"):
            ...
            self.end_headers()
            status_data = {
                "current_camera_index": camera_thread.index if camera_thread else cfg.CAM_SIDE_INDEX,
    ...
    def do_POST(self):
        ...
        if path == "/api/set_camera":
            ...
            with analysis_lock:
                ...
                # 2. Side Label OCR Reader (pytesseract)
                ...
                # 3. Multi-Way Reconciliation Policy
                # Note: Texture and banner are omitted for the side camera claim layer
                ...
                cv2.putText(annotated, f"QR SKU: {qr_sku} | Label SKU: {label_sku}", (30, 100),
                ...
                last_verdict_data = {
                    ...
                    "verdict_reason": f"QR: {qr_sku}, Label: {label_sku}",
                    "qr_sku": qr_sku,
                ...
                <div class="card">
                    ...
                    <div class="data-panel">
                        <div class="data-row">
                            ...
                            <span class="data-value sku-value" id="qrSku">—</span>
                        </div>
                        <div class="data-row">
                        ...
                        <div class="data-row">
                            ...
                            <span class="data-value sku-value" id="labelSku" style="color:var(--magenta);">—</span>
                        </div>
                        <div class="data-row">
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Mattress Side Camera QC Rig Server (Claim Layer Only)")
    parser.add_argument("--index", type=int, default=cfg.CAM_SIDE_INDEX, help="Camera device index (default from config)")
    parser.add_argument("--port", type=int, default=8003, help="HTTP server port (default: 8003)")
    parser.add_argument("--width", type=int, default=cfg.CLAIM_CAPTURE_W, help="Capture width (default from config)")
    parser.add_argument("--height", type=int, default=cfg.CLAIM_CAPTURE_H, help="Capture height (default from config)")
    args = parser.parse_args()

    print("======================================================================")
    print("MATTRESS SIDE CAMERA QUALITY CONTROL RIG (QR/OCR ONLY)")
    print("======================================================================")
