...
"""
============================================================
FILE: live_dimensions_stream2.py
PATH: top_camera/global shutter/live_dimensions_stream2.py
============================================================
DESCRIPTION:
  Upgraded Self-Contained Live Web Streaming & Measurement Rig.
  Features:
...
CREATED: 2026-08-01 | 02:55 PM

EDIT LOG:
------------------------------------------------------------
[2026-08-01 | 02:55 PM] - Initial creation of live_dimensions_stream2.py.
============================================================
"""
...
try:
    ...
    # Original SASH_S_THRESH=90 is too strict for live camera feed
    _cfg.SASH_S_THRESH = 60
    _cfg.SASH_V_THRESH = 60
    BANNER_OCR_AVAILABLE = True
    print("[stream2] banner.py OCR engine loaded (SASH thresholds relaxed to 60)")
except Exception as _be:
    BANNER_OCR_AVAILABLE = False
    print(f"[stream2] banner.py not available: {_be}")
...
class StreamerServer:
    def __init__(self, width=1920, height=1080, fps=30, webcam_index=8):
        ...
        self.height = height
        self.fps = fps
        self.webcam_index = webcam_index  # /dev/video8 = Logitech C920
    ...
    def _init_camera(self):
        # ── Try USB webcam first (Logitech C920 on /dev/video8) ──────────────
        try:
            ...
            if cap.isOpened():
                ...
                print(f"[stream2] Connected to USB webcam /dev/video{self.webcam_index} "
                      f"at {actual_w}x{actual_h}")
                return
        except Exception as e:
            print(f"[stream2] USB webcam /dev/video{self.webcam_index} failed: {e}")

        # ── Fallback: Picamera2 (IMX296 global shutter) ──────────────────────
        try:
            ...
            picam.start()
            self.picam2 = picam
            print("[stream2] Fallback: connected via Picamera2 (IMX296)")
        except Exception as e:
            print(f"[stream2] Picamera2 also failed: {e}")
...
# Logitech HD Pro Webcam C920 is on /dev/video8
streamer = StreamerServer(width=1920, height=1080, fps=30, webcam_index=8)
...
<head>
    <meta charset="UTF-8">
    <title>Live YOLO Dimension Stream 2</title>
...
<body>
    <div class="header">
        <h1>📐 Precision YOLO <span>Dimension Stream v2</span></h1>
...
if __name__ == '__main__':
    print("\n" + "="*65)
    print(" 🚀 PRECISION STREAM 2 ACTIVE: http://localhost:5000/")
    print("="*65 + "\n")
    try:
